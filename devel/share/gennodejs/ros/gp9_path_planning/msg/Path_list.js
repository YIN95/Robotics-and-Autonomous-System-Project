...
// (in-package gp9_path_planning.msg)
...
class Path_list {
  ...
  static deserialize(buffer, bufferOffset=[0]) {
    ...
    for (let i = 0; i < len; ++i) {
      data.list[i] = geometry_msgs.msg.Pose2D.deserialize(buffer, bufferOffset)
    }
    return data;
  }
  ...
  static datatype() {
    ...
    return 'gp9_path_planning/Path_list';
  }
  ...
  static Resolve(msg) {
    ...
    if (msg.list !== undefined) {
      ...
      for (let i = 0; i < resolved.list.length; ++i) {
        resolved.list[i] = geometry_msgs.msg.Pose2D.Resolve(msg.list[i]);
      }
    }
    else {
      resolved.list = []
